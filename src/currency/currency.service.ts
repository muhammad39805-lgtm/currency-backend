...
export class CurrencyService {
  ...
  async list() {
    if (!this.API_KEY) throw new HttpException('API Key not configured', 500);
  ...
  async convert(dto: ConvertDto) {
    ...
    let rate: number | undefined;
    rate = date ? res.data?.data?.[date]?.[to.toUpperCase()] : res.data?.data?.[to.toUpperCase()];

    if (!rate) throw new HttpException('Rate not found', 400);
  ...
  async history() {
    return this.conversionRepo.find({ order: { createdAt: 'DESC' } });
  }
}
