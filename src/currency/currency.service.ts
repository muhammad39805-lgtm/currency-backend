...
export class CurrencyService {
  ...
  constructor(
    ...
    // ✅ Safety check (VERY IMPORTANT for Vercel)
    if (!this.API_KEY) {
      throw new Error('CURRENCY_API_KEY is missing in environment variables');
    }
  }
  ...
  async convert(dto: ConvertDto) {
    ...
    if (!rate) {
      throw new HttpException(
        'Exchange rate not found for given currency/date',
